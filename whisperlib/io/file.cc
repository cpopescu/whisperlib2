...
absl::string_view File::AccessName(Access access) {
  switch (access) {
  case GENERIC_READ: return "GENERIC_READ";
  case GENERIC_WRITE: return "GENERIC_WRITE";
  case GENERIC_READ_WRITE: return "GENERIC_READ_WRITE";
  }
  return "UNKNOWN";
}
...
absl::string_view File::MoveMethodName(MoveMethod mm) {
  switch (mm) {
  case FILE_SET: return "FILE_SET";
  case FILE_CUR: return "FILE_CUR";
...
absl::StatusOr<std::string> File::ReadAsString(
    absl::string_view filename, size_t max_size) {
...
absl::StatusOr<size_t> File::WriteFromString(
...
absl::Status File::Set(absl::string_view filename, int fd) {
  RET_CHECK(!is_open()) << "Cannot set an already opened file: `"
                        << filename << "`";
...
absl::StatusOr<uint64_t> File::SetPosition(
    int64_t distance, MoveMethod move_method) {
  RET_CHECK(is_open());
  int whence = SEEK_SET;
  switch (move_method) {
...
absl::Status File::Rewind() {
  return SetPosition(0, FILE_SET).status();
}
  ...
  for (absl::string_view chunk : cord.Chunks()) {
    ...
    if (ABSL_PREDICT_FALSE(crt_cb != chunk.size())) {
      return status::InternalErrorBuilder()
        ...
        << chunk.size() << " bytes, but: " << crt_cb << " bytes were written "
        "to file `" << filename_ << "`";
    }
    cb += crt_cb;
    if (cb >= size_to_write) {
...
absl::StatusOr<size_t> File::WriteCordVec(
  ...
  if (ABSL_PREDICT_FALSE(cb < 0)) {
    absl::Status status = error::ErrnoToStatus(error::Errno())
      << "::writeev() failed for file: `" << filename_ << "` with: "
      << io_vec_pair.first.size() << " chunks and: " << io_vec_pair.second
      << " bytes.";
  }
  position_ += cb;
  size_ = std::max(size_, position_);
  if (ABSL_PREDICT_FALSE(size_t(cb) != io_vec_pair.second)) {
      return status::InternalErrorBuilder()
        ...
        << io_vec_pair.second << " bytes, but: " << cb << " bytes were written "
        "to file `" << filename_ << "`";
  }
