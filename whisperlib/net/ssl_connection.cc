...
std::string SslUtils::SslLastError() {
  ...
  errors.emplace_back(
      absl::StrCat("General error: ", error::ErrnoToString(error::Errno())));
...
absl::StatusOr<X509*> SslUtils::SslLoadCertificateFile(
...
absl::StatusOr<EVP_PKEY*> SslUtils::SslLoadPrivateKeyFile(
    absl::string_view filename) {
  std::string filename_str(filename);
  FILE* f = ::fopen(filename_str.c_str(), "r");
  if (f == nullptr) {
  ...
  if (nullptr == PEM_read_PrivateKey(f, &key, nullptr, nullptr) ||
      nullptr == key) {
    return status::InternalErrorBuilder()
           << "PEM_read_PrivateKey failed to load key from file: `" << filename
           << "`";
  }
  return key;
}
...
absl::StatusOr<SSL_CTX*> SslUtils::SslCreateContext(
    absl::string_view certificate_filename, absl::string_view key_filename) {
  SslLibraryInit();
  ...
  const long ssl_new_ctx_mode = ssl_ctx_mode | SSL_MODE_ENABLE_PARTIAL_WRITE |
...
SslAcceptor::SslAcceptor(Selector* selector, SslAcceptorParams params)
  ...
  tcp_acceptor_
      .set_filter_handler(
          absl::bind_front(&SslAcceptor::TcpAcceptorFilterHandler, this))
      .set_accept_handler(
          absl::bind_front(&SslAcceptor::TcpAcceptorAcceptHandler, this));
}
...
void SslAcceptor::Close() { tcp_acceptor_.Close(); }
...
void SslAcceptor::TcpAcceptorAcceptHandler(
  ...
  ssl_connection->Wrap(
      absl::WrapUnique(static_cast<TcpConnection*>(connection.release())));
  ssl_connection
      ->set_connect_handler(
          absl::bind_front(&SslAcceptor::SslConnectionConnectHandler, this,
                           ssl_connection.get()))
...
void SslAcceptor::SslConnectionCloseHandler(
    SslConnection* ssl_connection, const absl::Status& status,
...
absl::Status SslAcceptor::SslInitialize() {
  ...
  if (SSL_CTX_check_private_key(params_.ssl_params.ssl_context) != 1) {
...
void SslConnection::SetTcpConnectionHandlers() {
  tcp_connection_
      ->set_connect_handler(
          absl::bind_front(&SslConnection::TcpConnectionConnectHandler, this))
      .set_close_handler(
          absl::bind_front(&SslConnection::TcpConnectionCloseHandler, this))
      .set_read_handler(
          absl::bind_front(&SslConnection::TcpConnectionReadHandler, this))
      .set_write_handler(
          absl::bind_front(&SslConnection::TcpConnectionWriteHandler, this));
}
...
absl::Status SslConnection::Connect(const HostPort& remote_addr) {
  CHECK(tcp_connection_ == nullptr);
  tcp_connection_ =
      absl::make_unique<TcpConnection>(net_selector(), params_.tcp_params);
  SetTcpConnectionHandlers();
  set_state(CONNECTING);
  ...
  if (!connect_status.ok()) {
    ...
    return status::Annotate(connect_status,
                            "For underlying TCP connection of SSL connection");
  }
...
std::string SslConnection::ToString() const {
  std::string prefix(absl::StrCat(
      "[ SSL connection: ", state_name(), " server_side: ",
      is_server_side_.load(), " handshaked: ", handshake_finished_.load(),
...
absl::Status SslConnection::TcpConnectionReadHandler() {
  ...
  for (absl::string_view chunk : tcp_connection_->inbuf()->Chunks()) {
    ...
    if (write_size < int(chunk.size())) {
      return status::InternalErrorBuilder()
             << "BIO_write failed, closing connection: "
  ...
  while ((pending = std::max(BIO_pending(p_bio_read_), SSL_pending(p_ssl_))) >
         0) {
    ...
    size_t scratch_size =
        std::min(params_.tcp_params.block_size, size_t(pending));
    ...
    inbuf()->Append(absl::MakeCordFromExternal(absl::string_view(buffer, cb),
                                               clear_buffer.reset()));
  }
...
absl::Status SslConnection::TcpConnectionWriteHandler() {
  ...
  while ((pending = BIO_pending(p_bio_write_)) > 0) {
    ...
    if (cb < 0) {
      return status::InternalErrorBuilder()
             << "BIO_read failed, closing connection: "
             << SslUtils::SslLastError();
    }
  ...
  if (state() == FLUSHING && outbuf()->empty()) {
    ...
    net_selector_->RunInSelectLoop(
        [this]() { tcp_connection_->FlushAndClose(); });
  }
  return absl::OkStatus();
}
...
absl::Status SslConnection::InitializeSslVerificationIndex() {
  ...
  if (verification_index_.load() < 0) {
    verification_index_ =
        SSL_get_ex_new_index(0, (void*)"SSLConnection::verification_index",
...
absl::Status SslConnection::SslHandshake() {
  ...
  if (SSL_is_init_finished(p_ssl_)) {
    ...
    if (BIO_pending(p_bio_write_) > 0) {
      RETURN_IF_ERROR(RequestWriteEvents(true))
          << "After SslHandshake finished - delaying connect handler per "
             "pending "
             "writed: "
    ...
    set_state(CONNECTED);
    net_selector_->RunInSelectLoop(
        absl::bind_front(&SslConnection::CallConnectHandler, this));
    return absl::OkStatus();
  }
