#ifndef WHISPERLIB_STATUS_TESTING_H_
#define WHISPERLIB_STATUS_TESTING_H_
...
do {                                                                    \
  auto _res = (expr);                                                   \
  ::absl::Status _st = ::whisper::status::internal::AnyToStatus(_res);  \
  EXPECT_TRUE(_st.ok())                                                 \
    << "'" TEST_STATUS_MACROS_STRINGIFY(expr) "' failed with "          \
    << _st.ToString();                                                  \
} while (false)
...
ASSERT_OK_AND_ASSIGN_IMPL(                                              \
  TEST_STATUS_MACROS_CONCAT_NAME(_status_or_value, __COUNTER__),        \
  lhs, rexpr);

#define STATUSEXP(expr, ENUM, _st)                                      \
  "Expecting '" TEST_STATUS_MACROS_STRINGIFY(expr)                      \
  "' to fail with " TEST_STATUS_MACROS_STRINGIFY(ENUM)                  \
  ", but got " << _st.ToString()
...
#endif  // WHISPERLIB_STATUS_TESTING_H_
