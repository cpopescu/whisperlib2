...
class BlockingConcurrentQueue {
 ...
 public:
  ...
  inline std::pair<bool, T&&> enqueue(T&& item) {
    std::pair<bool, T&&> result = inner.enqueue(std::move(item));
    if ((details::likely)(result.first)) {
      sema->signal();
      return result;
    }
    return result;
  ...
  inline bool enqueue(producer_token_t const& token, T&& item) {
    std::pair<bool, T&&> result = inner.enqueue(token, std::move(item));
    if ((details::likely)(result.first)) {
      sema->signal();
      return result;
    }
  ...
  inline std::pair<bool, T&&> try_enqueue(T&& item) {
    std::pair<bool, T&&> result = inner.try_enqueue(std::move(item));
    if (result.first) {
      sema->signal();
      return result;
    }
  ...
  inline std::pair<bool, T&&> try_enqueue(producer_token_t const& token,
                                          T&& item) {
    std::pair<bool, T&&> result = inner.try_enqueue(token, std::move(item));
    if (result.first) {
      sema->signal();
      return result;
    }
